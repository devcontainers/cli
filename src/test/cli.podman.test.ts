...
describe('Dev Containers CLI using Podman', function () {
	...
	before('Install', async () => {
		...
		if (process.env.GITHUB_ACTIONS === 'true') {
			const userStorageConfig = path.join(process.env.HOME!, '.config', 'containers', 'storage.conf');
			let storageConfigContent: string;
			try {
				storageConfigContent = await readFile(userStorageConfig, 'utf8');
			} catch (err) {
				if (err?.code !== 'ENOENT') {
					throw err;
				}
				storageConfigContent = await readFile('/etc/containers/storage.conf', 'utf8');
			}
			...
			if (updatedStorageConfigContent !== storageConfigContent) {
				...
				await mkdir(path.dirname(userStorageConfig), { recursive: true });
				await writeFile(userStorageConfig, updatedStorageConfigContent);
			}
		}
