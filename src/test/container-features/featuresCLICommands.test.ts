...
✅ Passed:      'with_external_feature'`;
		...
		it('succeeds --skip-autogenerated and subset of features and --skip-idempotency-test', async function () {
			const collectionFolder = `${__dirname}/example-v2-features-sets/simple`;
			let success = false;
			let result: ExecResult | undefined = undefined;
			try {
				result = await shellExec(`${cli} features test -f color --skip-autogenerated --skip-idempotency-test --log-level trace ${collectionFolder}`);
				success = true;
