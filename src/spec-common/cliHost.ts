...
function skipHeader(headerSize: number, cb: SourceCallback<Buffer>, abort: Abort, data?: Buffer): number {
	...
	if (headerSize === 0) {
		...
		let removeSize = data.length - headerSize;
		data.copy(data, 0, removeSize);
		headerSize = 0;
		cb(null, data);
	} else {
...
function handleUnixSocketOnWindows(socket: net.Socket, socketPath: string): Duplex<Buffer, Buffer> {
	...
	(async () => {
		...
		socket.connect(port, '127.0.0.1', () => {
			...
			// Call pending source calls, if the pull-stream connection was pull-ed
			// before we got connected to the ssh-agent/gpg-agent server
