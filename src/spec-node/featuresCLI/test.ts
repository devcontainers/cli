...
export function featuresTestOptions(y: Argv) {
	return y
		.options({
			'features': { type: 'array', alias: 'f', conflicts: ['global-scenarios-only'], describe: 'Feature(s) to test as space-separated parameters. Omit to run all tests.  Cannot be combined with \'--global-scenarios-only\'.', },
			'global-scenarios-only': { type: 'boolean', conflicts: ['skip-scenarios'], default: false, description: 'Run only scenario tests under \'tests/_global\' .  Cannot be combined with \'-f\'.' },
