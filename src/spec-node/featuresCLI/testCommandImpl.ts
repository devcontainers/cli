...
async function runFeatureTests(args: FeaturesTestCommandInput, testResults: TestResult[] = []): Promise<TestResult[]> {
	const { baseImage, collectionFolder, remoteUser, cliHost, skipAutogenerated, skipScenarios, skipIdempotencyTest } = args;
	let { features } = args;
	...
	for (const feature of features) {
		...
		if (!skipIdempotencyTest) {
			log(`Executing idempotency test for feature '${feature}'...`, { prefix: '🧪' });
			await runIdempotencyTest(args, feature, testResults);
		}
