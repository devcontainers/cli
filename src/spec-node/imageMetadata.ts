...
export async function getImageBuildInfoFromImage(params: DockerResolverParameters | DockerCLIParameters, imageName: string, experimentalImageMetadata: boolean): Promise<ImageBuildInfo & { imageDetails: ImageDetails }> {
	...
	const metadata = getImageMetadata(imageDetails, experimentalImageMetadata);
	return {
		user,
		metadata,
		imageDetails,
...
export async function getImageBuildInfoFromDockerfile(params: DockerResolverParameters | DockerCLIParameters, dockerfile: string, experimentalImageMetadata: boolean) {
	return internalGetImageBuildInfoFromDockerfile(imageName => inspectDockerImage(params, imageName, true), dockerfile, experimentalImageMetadata);
}

export async function internalGetImageBuildInfoFromDockerfile(inspectDockerImage: (imageName: string) => Promise<ImageDetails>, dockerfile: string, experimentalImageMetadata: boolean): Promise<ImageBuildInfo> {
	// TODO: Other targets.
	...
	const metadata = imageDetails ? getImageMetadata(imageDetails, experimentalImageMetadata) : [];
	return {
		user,
		metadata,
	};
...
function getImageMetadata(imageDetails: ImageDetails, experimentalImageMetadata: boolean) {
