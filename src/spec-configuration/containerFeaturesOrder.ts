...
export function computeOverrideInstallationOrder(config: DevContainerConfig, features: FeatureSet[]) {
    ...
    for (const featureId of config.overrideFeatureInstallOrder!) {
        const feature = automaticOrder.find(feature => (feature.sourceInformation.type === 'oci' && feature.sourceInformation.featureRef.resource === featureId) || (feature.sourceInformation.type !== 'oci' && feature.features[0].name === featureId));
        if (!feature) {
